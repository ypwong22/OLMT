...
parser.add_option(
    "--compare_cases", dest="compare", default="", help="caseidprefix(es) to compare"
)
parser.add_option(
    "--ninst", dest="ninst", default=1, help="number of land model instances"
)
parser.add_option(
    "--mc_ensemble",
    dest="mc_ensemble",
...
parser.add_option(
    "--ng", dest="ng", default=256, help="number of groups to run in ensemble mode"
)
parser.add_option(
    "--parm_list",
    dest="parm_list",
...
parser.add_option("--model_root", dest="csmdir", default="", help="base CESM directory")
parser.add_option(
    "--compiler", dest="compiler", default="", help="compiler to use (pgi*, gnu)"
)
parser.add_option(
    "--mpilib",
    dest="mpilib",
...
parser.add_option(
    "--clean_build", action="store_true", default=False, help="Perform a clean build"
)
parser.add_option(
    "--cpl_bypass",
    dest="cpl_bypass",
...
parser.add_option("--machine", dest="machine", default="", help="machine to use")
parser.add_option("--np", dest="np", default=1, help="number of processors")
parser.add_option(
    "--walltime",
    dest="walltime",
    default=6,
...
parser.add_option(
    "--pio_version", dest="pio_version", default="2", help="PIO version (1 or 2)"
)
...
parser.add_option(
    "--nyears_final_spinup",
    dest="nyears_final_spinup",
    default="200",
    help="base no. of years for final spinup",
)
parser.add_option(
    "--nyears_transient",
    dest="nyears_transient",
    default=-1,
...
parser.add_option(
    "--surffile", dest="surffile", default="", help="Use specified surface data file"
)
parser.add_option(
    "--domainfile", dest="domainfile", default="", help="Domain file to use"
)
# parameters
parser.add_option(
    "--pft", dest="mypft", default=-1, help="Use this PFT (override site default)"
)
parser.add_option(
    "--siteparms",
    dest="siteparms",
...
parser.add_option(
    "--parm_file", dest="parm_file", default="", help="parameter file to use"
)
parser.add_option(
    "--parm_file_P", dest="parm_file_P", default="", help="parameter file to use"
)
parser.add_option(
    "--fates_paramfile",
    dest="fates_paramfile",
    default="",
    help="Fates parameter file to use",
)
parser.add_option(
    "--parm_vals", dest="parm_vals", default="", help="User specified parameter values"
)
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Daymet v4 downscaled GSWP3-v2 forcing with user-provided domain and surface data)",
)
parser.add_option(
    "--dailyvars",
    dest="dailyvars",
...
parser.add_option(
    "--trans_varlist", dest="trans_varlist", default="", help="Transient outputs"
)
parser.add_option(
    "--hist_mfilt_trans",
    dest="hist_mfilt",
...
parser.add_option(
    "--landusefile", dest="pftdynfile", default="", help="user-defined dynamic PFT file"
)
...
parser.add_option(
    ...
    dest="var_list_pft",
    default="",
    help="Comma-separated list of vars to output at PFT level",
)
parser.add_option(
    "--dryrun",
    dest="dryrun",
...
if int(options.mc_ensemble) != -1:
    ...
    numpy.savetxt(
        "mcsamples_" + options.mycaseid + "_" + str(options.mc_ensemble) + ".txt",
    ...
    if (row[0] in mysites) or (
        ...
        if options.nyears_transient == -1:
            ...
            if options.eco2_file != "":
                translen = (
                    translen - ncycle
                )  # if experiment sim, stop first transient at exp start yr - 1
            if options.cpl_bypass and (
                options.cruncep
                or options.gswp3
                or options.princeton
                or options.cruncepv8
                or options.gswp3_w5e5
            ):
        ...
        if (
            options.daymet4
        ):  # gswp3 v2 spatially-downscaled by daymet v4, usually together with user-defined domain and surface data
        ...
        if int(options.hist_mfilt_spinup) == -999:
            ...
            cmd_adsp = (
                cmd_adsp
                + " --hist_mfilt "
                + str(options.hist_mfilt_spinup)
                + " --hist_nhtfrq "
                + str(options.hist_nhtfrq_spinup)
            )
        if sitenum == 0:
            if options.exeroot != "":
        ...
        for c in case_list:
                    ...
                    if "perl" in s or "python" in s:
                        ...
                        timestr = (
                            ...
                            + str(
                                int(
                                    (
                                        float(options.walltime)
                                        - int(float(options.walltime))
                                    )
                                    * 60
                                )
                            )
                        ...
                        if mysubmit_type == "qsub":
                            ...
                            if "cades" in options.machine:
                                ...
                                output.write(
                                    "#SBATCH --mem=" + str(npernode * 2) + "G\n"
                                )
                                output.write(
                                    "#SBATCH --ntasks-per-node " + str(npernode) + "\n"
                                )
            ...
            if sitenum == 0 and (
                "ad_spinup" in c or (options.noad and "fn_spinup" in c)
            ):
                if "ad_spinup" in c:
                    mycasedir = (
                        caseroot
                        ...
                        + basecase
                        + "_"
                        + modelst.replace("CNP", "CN")
                        + "_ad_spinup"
                    )
                else:
                    mycasedir = caseroot + "/" + basecase + "_" + modelst
            if sitenum % npernode == 0 and (
            ...
            if "iniadjust" in c:
                ...
                if options.centbgc:
                    output.write(
                        "python adjust_restart.py --rundir "
                        + os.path.abspath(runroot)
                        + "/"
                        + ad_case
                        + "/run/ --casename "
                        + ad_case
            ...
            if "spinup_diags" in c:
                ...
                if options.machine == "cades":
                    output.write(
                        "scp -r ./plots/"
                        + mycaseid
                        + " acme-webserver.ornl.gov:~/www/single_point/plots\n"
                    )
            ...
            if sitenum == 0 and "fn_spinup" in c:
                ...
                output.write(
                    "cd " + runroot + "/" + basecase + "_" + modelst + "/run\n"
                )
                output.write(ad_exeroot + "/" + myexe + " &\n")
            ...
            # APW: possible alternative structure for these if statements, could help simplfy
                ...
                if sitenum == 0:
                    ...
                    simsuffix = "/run"
                    simsubmit = (
                        runroot + "/" + ad_case_firstsite + "/bld/" + myexe + " &\n"
                    )
                if options.crop:
                    output.write(
                        "cd "
            ...
            if "trans_diags" in c:
                ...
                output2 = open(
                    "./scripts/" + myscriptsdir + "/transdiag_" + site + ".csh", "w"
                )
                output.write("cd " + os.path.abspath(".") + "\n")
                ...
                output2.write(
                    plotcmd
                    + " --vars NEE,GPP,EFLX_LH_TOT,FSH,FPG,FPG_P,FPI,FPI_P,NPP,"
                    + "QOVER --seasonal --obs\n"
                )
                ...
                output2.write(
                    ...
                    + " --vars "
                    + diurnalvars
                    + " --obs --diurnal --dstart 1 --dend 59 --h1\n"
                )
                output2.write(
                    plotcmd
                    + " --vars "
                    + diurnalvars
                    + " --obs --diurnal --dstart 60 --dend 151 --h1\n"
                )
                output2.write(
                    plotcmd
                    + " --vars "
                    + diurnalvars
                    + " --obs --diurnal --dstart 152 --dend 243 --h1\n"
                )
                output2.write(
                    plotcmd
                    + " --vars "
                    + diurnalvars
                    + " --obs --diurnal --dstart 244 --dend 334 --h1\n"
                )
                ...
                output2.write(
                    plotcmd
                    + " --vars NEE,GPP,EFLX_LH_TOT,FSH,FPG,FPG_P,FPI,FPI_P,NPP,QOVER --obs\n"
                )
                # 1850-present
                output2.write(
                    plotcmd
                ...
                output.write(
                    "./scripts/" + myscriptsdir + "/transdiag_" + site + ".csh &\n"
                )
        ...
        if options.no_submit == False and options.ensemble_file != "":
            ...
            for thiscase in cases:
                job_depend_run = submit(
                    "scripts/" + myscriptsdir + "/ensemble_run_" + thiscase + ".pbs",
