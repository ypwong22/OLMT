...
if (options.clmpf_mode and options.clmpf_source_dir == ''):
    print('Error:  Model to be running in '+mylsm+'_INTERFACE_MODE=pflotran, but NO CLM_PFLOTRAN_SOURCE_DIR specified')
...
if (options.clmpf_source_dir!=''):
    ...
    print('Building '+mylsm+' with PFLOTRAN coupling codes from: ')
    os.system("echo $CLM_PFLOTRAN_SOURCE_DIR")
    #building CPPDEFS
    cppdefs = cppdefs + ' -D'+mylsm+'_PFLOTRAN -DCOLUMN_MODE'
    if (options.clmpf_mode):
       #running option
